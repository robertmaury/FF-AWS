...
import {
  to = aws_s3_bucket_ownership_controls.rmaury-lambda-datastore-e68
  id = "rmaury-lambda-datastore"
}
...
resource "aws_s3_bucket_public_access_block" "rmaury-lambda-datastore-1c9" {
  ...
  bucket                  = "${aws_s3_bucket_server_side_encryption_configuration.rmaury-lambda-datastore-91b.id}"
...
resource "aws_s3_bucket_server_side_encryption_configuration" "rmaury-lambda-datastore-91b" {
  bucket = "${aws_s3_bucket_public_access_block.rmaury-lambda-datastore-1c9.id}"
  rule {
...
resource "aws_s3_bucket_ownership_controls" "rmaury-lambda-datastore-e68" {
  bucket = "${aws_s3_bucket_public_access_block.rmaury-lambda-datastore-1c9.id}"
  rule {
